namespace ScytheBlog

open System
open System.IO
open ScytheBlog.BlogPosts
open ScytheBlog.FileHelpers
...
module Blog =
  ...
  let TransformFile template hasHeader (razor:ScytheBlog.Razor) prefix current target sourceUrl =
    let html =
      ...
      | (".fsx" | ".md") as ext ->
          ...
          if ext = ".fsx" then
              Literate.ProcessScriptFile(fsx.FileName, template, html.FileName, ?prefix=prefix, fsiEvaluator=FsiEvaluator())
          else
  ...
  let GeneratePostListing layouts template blogIndex model posts urlFunc needsUpdate infoFunc getPosts =
    for item in posts do
      ...
      let razor = ScytheBlog.Razor(layouts, Model = model)
