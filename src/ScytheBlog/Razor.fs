...
type Razor(layoutsRoot) =
    do
        ...
        config.Namespaces.Add("ScytheBlog") |> ignore
        ...
        config.BaseTemplateType <- typedefof<ScytheBlog.TemplateBaseExtensions<_>>
        config.Debug <- true
...
(*
    member x.LoadMarkdownFragment fragment =
        ...
        try
            ...
            let utmpl = (tmpl :?> ScytheBlog.TemplateBaseExtensions<_>)
...
*)
    ...
    member x.ProcessFile source (sourceUrl:string) =
      try
        x.ViewBag <- new DynamicViewBag()
        x.ViewBag.AddValue("SourceUrl", sourceUrl)
